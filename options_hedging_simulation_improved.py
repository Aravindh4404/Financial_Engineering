"""
OPTIONS HEDGING SIMULATION - IMPROVED VERSION
Added realistic vega rebalancing threshold

Key improvements:
1. Vega hedge now uses threshold (not daily rebalancing)
2. More realistic transaction costs
3. Better comparison across strategies
...
def simulate_delta_hedge(df, N=100, rebalance_threshold=0.20):
    ...
    df_result = pd.DataFrame(results)
    rebalances = (df_result['Stock_Trade'] > 0).sum()
    print(f"Final P&L: ₹{df_result['PnL'].iloc[-1]:,.2f}")
    print(f"Rebalanced {rebalances} times out of {len(df)} days")
    return df_result
...
def simulate_vega_hedge(df, N=100, rebalance_threshold=0.20):
    """
    Strategy 4: Delta + Vega Hedge with rebalancing threshold
    
    Rebalances BOTH vega (via hedge options) AND delta (via stock) only when
    positions drift beyond the threshold percentage.
    
    This is more realistic than daily rebalancing:
    - Reduces transaction costs
    - Vega changes slowly (mostly time decay and vol changes)
    - Focuses rebalancing on meaningful position changes
    """
    ...
    for i, row in df.iterrows():
        ...
        # STEP 1: Calculate target vega hedge position
        ...
        # Check if vega rebalancing needed
        if i == 0:
            vega_rebalance_needed = True
        else:
            if last_target_hedge_opts != 0:
                pct_change_vega = abs((target_hedge_opts - last_target_hedge_opts) / last_target_hedge_opts)
            else:
                pct_change_vega = abs(target_hedge_opts) if target_hedge_opts != 0 else 0
            vega_rebalance_needed = pct_change_vega > rebalance_threshold

        # Execute hedge option trade if needed
        hedge_trade = 0
        if vega_rebalance_needed:
            ...
            last_target_hedge_opts = target_hedge_opts

        # STEP 2: Calculate target delta hedge position
        short_delta = -1 * row['Delta'] * N
        hedge_delta = hedge_opt_pos * row['Hedge_Delta']
        current_net_delta = short_delta + hedge_delta
        target_stock = -current_net_delta

        # Check if delta rebalancing needed
        if i == 0:
            delta_rebalance_needed = True
        else:
            if last_target_stock != 0:
                pct_change_delta = abs((target_stock - last_target_stock) / last_target_stock)
            else:
                pct_change_delta = abs(target_stock) if target_stock != 0 else 0
            delta_rebalance_needed = pct_change_delta > rebalance_threshold

        # Execute stock trade if needed
        stock_trade = 0
        if delta_rebalance_needed:
            stock_trade = target_stock - stock_pos
            stock_trade_cost = stock_trade * S
            cash -= stock_trade_cost
            stock_pos = target_stock
    ...
    vega_rebalances = (df_result['Hedge_Trade'] > 0).sum()
    delta_rebalances = (df_result['Stock_Trade'] > 0).sum()
    
    print(f"Final P&L: ₹{df_result['PnL'].iloc[-1]:,.2f}")
    print(f"Vega rebalances (hedge options): {vega_rebalances} times out of {len(df)} days")
    print(f"Delta rebalances (stock): {delta_rebalances} times out of {len(df)} days")
...
def create_summary_statistics(strategies_dict, option_type):
    ...
    for name, df in strategies_dict.items():
        ...
        # Count rebalancing activity
        stock_rebalances = (df['Stock_Trade'] > 0).sum()
        hedge_rebalances = (df['Hedge_Trade'] > 0).sum()
        ...
        summary_data.append({
            ...
            'Stock_Rebalances': stock_rebalances,
            'Hedge_Opt_Rebalances': hedge_rebalances
        })
...
def main():
    ...
    print("\n" + "=" * 70)
    print("OPTIONS HEDGING - IMPROVED VERSION")
    print("Realistic Vega Hedging with Rebalancing Threshold")
    print("=" * 70)
    ...
    if os.path.exists(CALL_MAIN_FILE) and os.path.exists(CALL_HEDGE_FILE):
        ...
        call_strategies = {
            ...
            'Delta-Vega': simulate_vega_hedge(df_call, N_CONTRACTS, rebalance_threshold=0.20)
        }
    ...
    if os.path.exists(PUT_MAIN_FILE) and os.path.exists(PUT_HEDGE_FILE):
        ...
        put_strategies = {
            ...
            'Delta-Vega': simulate_vega_hedge(df_put, N_CONTRACTS, rebalance_threshold=0.20)
        }
